    ...
    <section class="safe-area-md">
        <div class="mx-auto max-w-6xl">
            <Breadcrumb :items="[
                ...
                ...(post ? [{ name: post.title, url: `/commerces/${post.slug}` }] : [])
            ]" />
            ...
            <article v-else-if="post" class="mt-8">
                ...
                <CommerceInfos :post="post" class="mt-4" />
...
import CommerceInfos from '~/components/organisms/front-office/CommerceInfos.vue';
import MediaCarousel from '~/components/organisms/front-office/MediaCarousel.vue';

const route = useRoute()
const { post, loading } = await usePost('commerce', route.params.slug as string)

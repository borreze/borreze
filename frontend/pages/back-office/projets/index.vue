        ...
        <Table class="mt-4" :loading="loading" :items="posts" titleKey="title" descriptionKey="abstract" :columns="[
            ...
            <template #cell-progression="{ item }">
                <div class="flex flex-wrap items-center gap-2">
                    <Pill v-if="item?.progression"
                        :label="POST_PROGESSIONS_OBJECTS.find(prog => prog.value === item?.progression)?.label || item?.progression"
                        :color="POST_PROGESSIONS_OBJECTS.find(prog => prog.value === item?.progression)?.color"
                        size="sm" />
                </div>
            </template>

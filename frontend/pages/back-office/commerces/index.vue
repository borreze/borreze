<template>
    <div>
        <Teleport defer to="#page-heading">
            <h1 class="title-main line-clamp-1">Commerces</h1>
            <Pill v-if="pagination?.count" :label="pagination?.count" variant="light" size="md" />
        </Teleport>
        <Teleport defer to="#page-actions">
            <Button v-if="authStore.canIDo('post', 'create')" as="link" href="/back-office/commerces/ajouter"
        ...
        <Table class="mt-4" :loading="loading" :items="posts" titleKey="title" descriptionKey="abstract" :columns="[
            ...
            { label: 'Voir', icon: 'ic:outline-remove-red-eye', variant: 'ghost' as ComponentVariant, buildLink: (item: PostAttributesFrontend) => `/commerces/${item.slug}`, external: true },
            ...(authStore.canIDo('post', 'read') ? [{ label: 'Modifier', icon: 'ic:baseline-edit', variant: 'primary' as ComponentVariant, buildLink: (item: PostAttributesFrontend) => `/back-office/commerces/${item.id}` }] : []),
        ]">
...
const { posts, pagination, loading, setPage, setOrder, resetOrder, setSearch } = await usePosts('commerce')

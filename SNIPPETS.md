...
<template>
    <div class="p-6">
        <Table :items="tickets" :columns="columns" route-pattern="/tickets/{id}" :formatters="formatters"
            :loading="loading" :error="error" :is-empty="tickets.length === 0" empty-message="Aucun ticket trouvé"
            show-bell-icon>
            <template #cell-priority="{ value }">
                <span :class="{
                    'text-red-600 font-bold': value === 'Haute',
                    'text-yellow-600 font-semibold': value === 'Moyenne',
                    'text-green-600': value === 'Basse'
                }">
                    {{ value }}
                </span>
            </template>
...
// Define the columns displayed
const columns = [
    { key: 'ref', label: 'Ticket', cellClass: 'font-semibold' },
    { key: 'subject', label: 'Sujet' },
    { key: 'priority', label: 'Priorité' },
    { key: 'created_at', label: 'Créé le', formatter: 'formatDate' },
]
